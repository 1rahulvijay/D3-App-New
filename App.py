...
app = Flask(__name__)
CORS(app)
# Securely load database URI and secret key from environment variables
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'oracle+oracledb://your_username:your_password@host:port/service_name')
...
class Comment(db.Model):
    __tablename__ = 'BCM_COMMENTS'  # Map to existing Oracle table
    id = db.Column(db.Integer, primary_key=True)  # Assumes sequence/trigger for auto-increment
    chart_id = db.Column(db.String(50), nullable=False)
    page = db.Column(db.String(50), nullable=False)
    text = db.Column(db.Text, nullable=False)  # Maps to CLOB or VARCHAR2
    ...
    try:
        ...
        if 'chart_id' in data:
            if not data['chart_id']:
                return jsonify({"error": "chart_id cannot be empty"}), 400
            comment.chart_id = data['chart_id'][:50]
        if 'page' in data:
            if not data['page']:
                return jsonify({"error": "page cannot be empty"}), 400
            comment.page = data['page'][:50]
        if 'text' in data:
        ...
        if 'user' in data:
            comment.user = data['user'][:100] if data['user'] else 'Anonymous'
        if 'reason' in data:
            comment.reason = data['reason']
        if 'exclusion' in data:
            comment.exclusion = data['exclusion']
        if 'why' in data:
            comment.why = data['why']
        if 'quick_fix' in data:
            comment.quick_fix = data['quick_fix']
        if 'to_do' in data:
            comment.to_do = data['to_do']

        db.session.commit()
        socketio.emit('update_comment', {
            'id': comment.id,
            'chart_id': comment.chart_id,
            'page': comment.page,
            'text': comment.text,
        ...
        return jsonify({"message": "Comment updated successfully", "id": comment.id}), 200
    except DatabaseError as e:
        logger.error(f"Oracle database error updating comment {id}: {e}")
        db.session.rollback()
        return jsonify({"error": "Database error occurred"}), 500
    except Exception as e:
        logger.error(f"Error updating comment {id}: {e}")
        db.session.rollback()
...
def delete_comment(id):
    try:
        ...
        db.session.commit()
        socketio.emit('delete_comment', {'id': id}, namespace='/annotations')
        return jsonify({"message": "Comment deleted successfully", "id": id}), 200
    except DatabaseError as e:
        logger.error(f"Oracle database error deleting comment {id}: {e}")
        db.session.rollback()
        return jsonify({"error": "Database error occurred"}), 500
    except Exception as e:
        logger.error(f"Error deleting comment {id}: {e}")
        db.session.rollback()
